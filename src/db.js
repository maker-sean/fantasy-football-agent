...
async function upsertPlayers(players) {
  ...
  const exp = players.map(p => p.years_exp ?? null);
  const ages = players.map(p => p.age ?? null);
  ...
  await query(
    `insert into players (player_id, full_name, position, team,
                          ...
                          years_exp, age, updated_at)
     select p.player_id, p.full_name, p.position, p.team,
            p.injury_status, p.injury_body_part, p.player_status,
            p.injury_notes, p.depth_chart_order, p.depth_chart_position,
            p.years_exp, p.age, now()
     from unnest($1::text[], $2::text[], $3::text[], $4::text[],
                 $5::text[], $6::text[], $7::text[],
                 $8::text[], $9::int[], $10::text[], $11::int[], $12::int[])
       as p(player_id, full_name, position, team,
            injury_status, injury_body_part, player_status,
            injury_notes, depth_chart_order, depth_chart_position, years_exp, age)
     on conflict (player_id) do update
       set full_name            = excluded.full_name,
           ...
           age                  = excluded.age,
           updated_at = now()`,
    [ids, names, positions, teams, injuries, parts, statuses, notes, depthOrder, depthPos, exp, ages]
  );
